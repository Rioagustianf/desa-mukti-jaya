...
export async function GET(
  ...
  await dbConnect();
  const data = await Prestasi.findById(params.id);
  if (!data)
...
export async function PUT(
  ...
  const updated = await Prestasi.findByIdAndUpdate(params.id, prestasiData, {
    new: true,
  });
  if (!updated)
    return NextResponse.json(
...
export async function DELETE(
  ...
  await dbConnect();
  const deleted = await Prestasi.findByIdAndDelete(params.id);
  if (!deleted)
    return NextResponse.json(
      { success: false, message: "Not found" },
      { status: 404 }
